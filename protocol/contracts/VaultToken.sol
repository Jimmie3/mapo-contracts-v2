...
contract VaultToken is IVaultToken, AccessControlEnumerable, ERC20Burnable {
    ...
    function deposit(uint256 _amount, address _to) external override onlyRole(MANAGER_ROLE) {
    ...
    function withdraw(uint256 _vaultAmount, address _to) external override onlyRole(MANAGER_ROLE) {
    ...
    function increaseVaultBalance(uint256 amount) external override onlyRole(MANAGER_ROLE) {
        totalVault += amount;
    }

    function reduceVaultBalance(uint256 amount) external override onlyRole(MANAGER_ROLE) {
        if(totalVault < amount) totalVault = 0;
        else totalVault -= amount;
    }
}
